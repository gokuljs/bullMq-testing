...
const addJobs = async () => {
    ...
    await queueInstance.add("job1", { number: 300 }, {
        repeat: {
            every: 1000,
            limit: 5,
        },
    });
    await queueInstance.add("job3", { number: 40000000000 }, {
        attempts: 3,
        backoff: {
            type: "exponential",
            delay: 600,
...
await addJobs();
const WorkerInstance = new Worker(QUEUE_NAME, async (job) => {
    if (job.data.number) {
...
const testInstance = new Worker(QUEUE_NAME, async (job) => {
    if (job.data.number) {
        return intensiveSort(job.data.number);
    }
}, {
